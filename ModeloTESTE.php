...
// Função para buscar o valor correspondente no XML com base no índice
function buscarValorPorIndice($xml, $campo, $indice) {
    // Verifica se o campo existe no XML
    if (isset($xml->item0->response->categories->{$campo}->unidade[$indice])) {
        // Retorna o texto associado ao valor encontrado
        return (string) $xml->item0->response->categories->{$campo}->unidade[$indice]->text;
    } else {
...
$lotação = buscarValorPorIndice($xml, 'G01Q010', 0); 

$texto = "Eu, $nome, cargo $cargo, RG $rg, lotado(a) na $lotação, da Polícia Científica do município de $municipio, venho requerer autorização para a concessão de:";
